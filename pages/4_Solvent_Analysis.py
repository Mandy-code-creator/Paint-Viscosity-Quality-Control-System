import streamlit as stimport pandas as pdimport numpy as npimport graphvizimport ioimport matplotlib.pyplot as plt

from docx import Documentfrom docx.shared import Inchesfrom docx.enum.text import WD_ALIGN_PARAGRAPH
...
st.title("🌳 Solvent Consumption Hierarchy")st.markdown("Coil-level analysis of solvent utilization mapping from Vendor ""to specific Resin, Coating Position, and Solvent types.")
...
if not st.session_state.get("raw_data_loaded", False):st.warning("⚠️ No data loaded. Please upload your dataset on the Main App page.")st.stop()

group_a = st.session_state.get("group_a_data", pd.DataFrame()).copy()

if group_a.empty:st.error("❌ The dataset is empty. Please check the uploaded file.")st.stop()
...
if "塗裝位置" not in group_a.columns:group_a["塗裝位置"] = "Unknown"

pos_mapping = {"TP": "Primer", "正底漆": "Primer","BP": "Primer", "背底漆": "Primer","TF": "Top Finish", "正面漆": "Top Finish","BF": "Back Finish", "背面漆": "Back Finish"}

group_a["Position_UI"] = group_a["塗裝位置"].map(pos_mapping).fillna(group_a["塗裝位置"])

for col in ["Vendor", "Resin", "Solvent_Type"]:if col not in group_a.columns:group_a[col] = "Unknown"
...
if not vendor_list:st.warning("⚠️ No Vendor data available.")st.stop()
...
resin_filter_list = ["All Resins"] + sorted(vendor_df["Resin"].dropna().unique().tolist())

selected_resin_filter = st.sidebar.selectbox("Select Resin Group:",resin_filter_list)

Lọc Resin theo lựa chọn

if selected_resin_filter != "All Resins":vendor_df = vendor_df[vendor_df["Resin"] == selected_resin_filter].copy()
...
position_list = ["All Positions"] + sorted(vendor_df["Position_UI"].dropna().unique().tolist())
...
if selected_position != "All Positions":filtered_df = filtered_df[filtered_df["Position_UI"] == selected_position].copy()

Enforce Grade A-B filter logic

if "Grade" in filtered_df.columns:filtered_df = filtered_df[filtered_df["Grade"].isin(["A", "B", "A-B"])].copy()

if filtered_df.empty:st.warning(f"⚠️ No valid data available for Vendor: {selected_vendor}, "f"Resin: {selected_resin_filter}, "f"Position: {selected_position}.")st.stop()
...
if not all(col in filtered_df.columns for col in required_cols):st.error(f"❌ Missing required columns: {', '.join(required_cols)}")st.stop()

for col in required_cols:filtered_df[col] = pd.to_numeric(filtered_df[col], errors="coerce")

Keep valid dilution records only

filtered_df = filtered_df[(filtered_df["塗料重量"] > 0)& (filtered_df["添加重量"] > 0)& (filtered_df["黏度(秒)"] > filtered_df["黏度(秒)_1"])].copy()

if filtered_df.empty:st.warning("⚠️ No valid dilution records after data cleaning.")st.stop()
...
filtered_df["Dilution_Efficiency"] = filtered_df["Delta_V"] / filtered_df["Solvent_Ratio_Percent"]

filtered_df = filtered_df.replace([np.inf, -np.inf], np.nan).dropna(subset=["Solvent_Ratio_Percent", "Delta_V", "Kg_per_1s", "Dilution_Efficiency"])

if filtered_df.empty:st.warning("⚠️ No valid records remain after calculations.")st.stop()

=========================================================

4. HIERARCHY SUMMARY (ADVANCED LOGIC WITH COATING POSITION)

=========================================================

def calculate_advanced_metrics(group):if len(group) < 5:return pd.Series({"Opt_Eff": group["Dilution_Efficiency"].median(),"Sat_Limit": np.nan})
...
df_calc["Ratio_Bin"] = pd.cut(
    df_calc["Solvent_Ratio_Percent"], bins=bins, labels=labels, include_lowest=True
)
...
tree_base = filtered_df.groupby(["Resin", "Position_UI", "Solvent_Type"], observed=False).agg(Total_Paint=("塗料重量", "sum"),Total_Solvent=("添加重量", "sum"),Avg_Visc_Before=("黏度(秒)", "mean"),Avg_Visc_After=("黏度(秒)_1", "mean")).reset_index()

advanced_metrics = filtered_df.groupby(["Resin", "Position_UI", "Solvent_Type"], observed=False).apply(calculate_advanced_metrics).reset_index()

tree_summary = pd.merge(tree_base, advanced_metrics, on=["Resin", "Position_UI", "Solvent_Type"])

tree_summary = tree_summary[tree_summary["Total_Paint"] > 0].sort_values(by=["Resin", "Total_Paint"], ascending=[True, False])

if tree_summary.empty:st.warning("⚠️ No valid paint consumption data available.")st.stop()
...
graph.attr(rankdir="LR",splines="curved",nodesep="0.2",ranksep="0.6",bgcolor="transparent")

graph.attr("node", shape="none", margin="0", fontname="Arial")graph.attr("edge", color="#B0B0B0", penwidth="1.2", arrowsize="0.7")

total_vendor_paint = tree_summary["Total_Paint"].sum()total_vendor_solv = tree_summary["Total_Solvent"].sum()avg_delta_v = filtered_df["Delta_V"].mean()total_dilution_base = filtered_df["Dilution_Base_kg"].sum()

avg_solvent_ratio = (total_vendor_solv / total_dilution_base * 100if total_dilution_base > 0 else 0)

date_range_str = "All Available Data"date_cols = [col for col in filtered_df.columns if "date" in col.lower() or "日期" in col.lower() or "time" in col.lower()]

if date_cols:try:date_col = date_cols[0]min_date = pd.to_datetime(filtered_df[date_col], errors="coerce").min()max_date = pd.to_datetime(filtered_df[date_col], errors="coerce").max()
...
--- LEVEL 0: ROOT NODE (VENDOR) ---

center_html = f"""<TABLE BORDER="0" CELLBORDER="0" CELLSPACING="0" CELLPADDING="6">

  <TR>
    <TD BGCOLOR="#00BFFF" STYLE="ROUNDED" ALIGN="CENTER">
      <FONT COLOR="white" POINT-SIZE="12"><B>VENDOR: {selected_vendor}</B></FONT>
    </TD>
  </TR>
  <TR>
    <TD BGCOLOR="#F8F9FA" STYLE="ROUNDED" ALIGN="CENTER">
      <FONT POINT-SIZE="10" COLOR="#333333">Period: <B>{date_range_str}</B><BR/>
      ...
      </FONT><FONT COLOR="#D9534F" POINT-SIZE="11"><B>Avg. Solvent Ratio: {avg_solvent_ratio:.2f}%</B></FONT>
    </TD>
  </TR>
</TABLE>"""
graph.node("Root", f"<{center_html}>")

--- LEVEL 1, 2 & 3: RESIN -> POSITION -> SOLVENT ---

for resin in tree_summary["Resin"].unique():resin_id = f"resin_{resin}"resin_data = tree_summary[tree_summary["Resin"] == resin].copy()
...
resin_html = f"""<TABLE BORDER="0" CELLBORDER="0" CELLSPACING="0" CELLPADDING="5">
  <TR>
    <TD BGCOLOR="#E6F2FF" STYLE="ROUNDED" BORDER="1" COLOR="#00BFFF" ALIGN="CENTER">
      <FONT COLOR="#005A9E" POINT-SIZE="11"><B>RESIN: {resin}</B></FONT><BR/>
      <FONT COLOR="#555555" POINT-SIZE="10">{resin_paint_sum:,.0f} kg Paint</FONT><BR/>
      <FONT COLOR="#D9534F" POINT-SIZE="10">{resin_solvent_sum:,.0f} kg Solvent</FONT>
    </TD>
  </TR>
</TABLE>"""
...
for pos in resin_data["Position_UI"].unique():
    ...
    pos_data = resin_data[resin_data["Position_UI"] == pos].copy()
    
    pos_paint_sum = pos_data["Total_Paint"].sum()
    
    pos_html = f"""<TABLE BORDER="0" CELLBORDER="0" CELLSPACING="0" CELLPADDING="4">
      <TR>
        <TD BGCOLOR="#FFF7ED" STYLE="ROUNDED" BORDER="1" COLOR="#F97316" ALIGN="CENTER">
          <FONT COLOR="#C2410C" POINT-SIZE="10"><B>POS: {pos}</B></FONT><BR/>
          <FONT COLOR="#555555" POINT-SIZE="9">{pos_paint_sum:,.0f} kg</FONT>
        </TD>
      </TR>
    </TABLE>"""
    ...
    for idx, row in pos_data.iterrows():
        ...
        leaf_html = f"""<TABLE BORDER="0" CELLBORDER="0" CELLSPACING="0" CELLPADDING="5">
          <TR>
...
st.markdown("---")st.subheader("📉 Dilution Efficiency & Saturation Analysis")

st.caption("Dilution Efficiency = Viscosity Drop ÷ Solvent Blending Ratio. ""Lower values indicate that additional solvent produces less ""viscosity reduction.")

col1, col2, col3 = st.columns(3)

with col1:resin_options = sorted(filtered_df["Resin"].dropna().unique().tolist())selected_resin = st.selectbox("Select Resin for Saturation Analysis:", resin_options, key="sat_resin")

analysis_resin_df = filtered_df[filtered_df["Resin"] == selected_resin].copy()

with col2:pos_options = sorted(analysis_resin_df["Position_UI"].dropna().unique().tolist())selected_pos = st.selectbox("Select Position:", pos_options, key="sat_pos")

analysis_pos_df = analysis_resin_df[analysis_resin_df["Position_UI"] == selected_pos].copy()

with col3:solvent_options = sorted(analysis_pos_df["Solvent_Type"].dropna().unique().tolist())selected_solvent = st.selectbox("Select Solvent:", solvent_options, key="sat_solvent")

saturation_df = analysis_pos_df[analysis_pos_df["Solvent_Type"] == selected_solvent].copy()

fig_efficiency = Nonebaseline_efficiency = Nonewarning_ratio = Nonestop_ratio = Nonebaseline_range_label = "N/A"baseline_records = 0

if len(saturation_df) < 5:st.warning("⚠️ Historical records are insufficient for saturation analysis (minimum 5 records required).")else:bins = [0, 3, 5, 7, 9, 11, np.inf]labels = ["0–3%", "3–5%", "5–7%", "7–9%", "9–11%", ">11%"]midpoints = [1.5, 4, 6, 8, 10, 12]

saturation_df["Ratio_Bin"] = pd.cut(
    saturation_df["Solvent_Ratio_Percent"], bins=bins, labels=labels, include_lowest=True
)

efficiency_summary = saturation_df.groupby("Ratio_Bin", observed=False).agg(
    Records=("Dilution_Efficiency", "size"),
    Median_Efficiency=("Dilution_Efficiency", "median")
).reset_index()
...
if efficiency_summary.empty:
    ...
    efficiency_summary["Efficiency_Retention_Percent"] = (
        efficiency_summary["Median_Efficiency"] / baseline_efficiency * 100
    )
    ...
    warning_ratio = warning_rows["Ratio_Midpoint"].iloc[0] if not warning_rows.empty else ratio_p90
    stop_ratio = stop_rows["Ratio_Midpoint"].iloc[0] if not stop_rows.empty else ratio_p95
    stop_ratio = max(stop_ratio, warning_ratio)

    # MATPLOTLIB CHART
    fig_efficiency, ax = plt.subplots(figsize=(11.5, 5.8), dpi=180)
    ...
    ax.plot(x_values, y_values, marker="o", linewidth=2, markersize=6, color="#4F6DFF")

    for x, y, record in zip(x_values, y_values, record_values):
        ax.annotate(
            str(record), xy=(x, y), xytext=(0, 10), textcoords="offset points",
    ...
    if abs(stop_ratio - warning_ratio) < 0.2:
        ...
        ax.annotate(
            ...
            xytext=(8, 0), textcoords="offset points", color=alert_color,
    ...
    ax.text(
        ...
        bbox=dict(facecolor="white", edgecolor="#CCCCCC", pad=5, alpha=0.9), va="bottom", ha="left"
    )

    ax.set_title(
        "Dilution Efficiency vs Solvent Ratio\n"
        f"Resin: {selected_resin} | Position: {selected_pos} | Solvent: {selected_solvent}",
    ...
    with st.expander("View Saturation Analysis Data"):
        display_efficiency = efficiency_summary[
            ["Ratio_Bin", "Records", "Median_Efficiency", "Efficiency_Retention_Percent"]
        ].copy()

        display_efficiency.columns = [
            "Solvent Ratio Range", "Records", "Median Efficiency (s/%)", "Efficiency Retention (%)"
        ]
...
try:graph.attr(dpi="300")graph_png = graph.pipe(format="png")
...
title = doc.add_heading("Solvent Consumption & Viscosity Control Report", level=0)
...
hierarchy_caption = doc.add_paragraph("Figure 1. Solvent consumption hierarchy by resin, position, and solvent type.")
hierarchy_caption.alignment = WD_ALIGN_PARAGRAPH.CENTER

if fig_efficiency is not None:
    chart_stream = io.BytesIO()
    fig_efficiency.savefig(chart_stream, format="png", dpi=300, bbox_inches="tight", facecolor="white")
    chart_stream.seek(0)
    ...
    doc.add_paragraph(
        ...
        "material condition does not have sufficient historical data."
    )
...
st.download_button(
    ...
    data=doc_io,
    file_name=f"Solvent_Viscosity_Report_{selected_vendor}_{selected_resin}_{selected_pos}_{selected_solvent}.docx",
