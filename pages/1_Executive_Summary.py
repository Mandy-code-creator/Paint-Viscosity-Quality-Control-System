...
# --- 7. PAINT VS SOLVENT TREND ANALYSIS ---
st.markdown("---")
st.subheader("📈 Xu hướng & Mối quan hệ giữa Sơn và Dung môi")
st.caption("Biểu đồ phân tán (Scatter plot) thể hiện tỷ lệ dung môi được thêm vào so với lượng sơn ban đầu. Đường xu hướng (Trendline) giúp nhận diện rõ tỷ lệ hao phí dung môi theo từng Nhà cung cấp và Loại dung môi.")

# Đảm bảo cột Solvent_Type tồn tại để tránh lỗi
if 'Solvent_Type' not in group_a.columns:
    group_a['Solvent_Type'] = 'Unknown/Default'

# 1. Bộ lọc (Filters) cho biểu đồ xu hướng
c_trend1, c_trend2 = st.columns(2)
with c_trend1:
    trend_vendors = st.multiselect(
        "🏭 Chọn Nhà cung cấp (Vendor)", 
        options=group_a['Vendor'].unique(), 
        default=group_a['Vendor'].unique(), 
        key='trend_vendor'
    )
with c_trend2:
    trend_solvents = st.multiselect(
        "💧 Chọn Loại dung môi (Solvent Type)", 
        options=group_a['Solvent_Type'].unique(), 
        default=group_a['Solvent_Type'].unique(), 
        key='trend_solvent'
    )

# Lọc dữ liệu dựa trên lựa chọn
trend_data = group_a[
    (group_a['Vendor'].isin(trend_vendors)) &
    (group_a['Solvent_Type'].isin(trend_solvents))
].copy()

# 2. Vẽ biểu đồ
if not trend_data.empty:
    # Cần thư viện statsmodels để vẽ trendline='ols' (Cài đặt: pip install statsmodels)
    try:
        fig_trend = px.scatter(
            trend_data,
            x='塗料重量',               # Trọng lượng sơn
            y='添加重量',               # Trọng lượng dung môi
            color='Vendor',             # Phân biệt màu theo Vendor
            facet_col='Solvent_Type',   # Chia biểu đồ thành nhiều cột theo loại dung môi
            trendline='ols',            # Vẽ đường xu hướng tuyến tính (Ordinary Least Squares)
            labels={
                '塗料重量': 'Trọng lượng sơn (kg)',
                '添加重量': 'Trọng lượng dung môi (kg)',
                'Vendor': 'Nhà cung cấp',
                'Solvent_Type': 'Loại dung môi'
            },
            title="Mối quan hệ tuyến tính: Sơn vs Dung môi tiêu thụ",
            opacity=0.7
        )
        
        # Tối ưu hóa giao diện biểu đồ
        fig_trend.update_layout(height=500, hovermode="closest")
        # Cho phép các trục Y độc lập nếu lượng dung môi giữa các loại khác biệt quá lớn
        fig_trend.update_yaxes(matches=None) 
        
        st.plotly_chart(fig_trend, use_container_width=True)
        
        st.info("""
        💡 **Hướng dẫn đọc biểu đồ dành cho Quản lý:**
        * **Các điểm chấm (Dots):** Mỗi điểm là một mẻ pha trộn thực tế. Điểm phân tán càng rộng chứng tỏ quy trình pha dung môi càng thiếu ổn định.
        * **Đường xu hướng (Trendline):** Thể hiện tỷ lệ trung bình. Đường thẳng nào có **độ dốc cao hơn** nghĩa là Vendor/Loại dung môi đó đang **tiêu tốn nhiều dung môi hơn** cho cùng một lượng sơn.
        * **Biểu đồ chia cột (Facets):** Giúp sếp dễ dàng so sánh chéo hiệu suất giữa các loại dung môi (Solvent Type) khác nhau.
        """)
    except Exception as e:
        st.error("Lỗi khi vẽ đường xu hướng. Vui lòng đảm bảo máy chủ/môi trường đã cài đặt thư viện `statsmodels` (chạy lệnh: `pip install statsmodels`).")
        # Fallback vẽ biểu đồ không có trendline nếu thiếu thư viện
        fig_trend_fallback = px.scatter(
            trend_data, x='塗料重量', y='添加重量', color='Vendor', facet_col='Solvent_Type'
        )
        st.plotly_chart(fig_trend_fallback, use_container_width=True)

else:
    st.warning("Không có dữ liệu cho các bộ lọc đã chọn.")
