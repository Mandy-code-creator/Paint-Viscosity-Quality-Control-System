...
# =========================================================
def export_chart_to_word(selected_resin, selected_pos, selected_vendor, selected_solvent, system_df):
    doc = Document()
...
def process_data(df):
    ...
    required_columns = [
        ...
        "Resin", "Vendor", "Solvent_Type", "塗料批號"
    ]
    ...
    system_batch_counts = data.groupby(group_cols)["塗料批號"].transform("nunique")
...
def build_saturation_profile(df):
    ...
    profile = sat_df.groupby("Ratio_Zone", observed=False).agg(
    ...
    if not valid_profile.empty:
        ...
        profile["Efficiency_vs_Baseline_%"] = (
            profile["Sensitivity_Median"] / baseline_sensitivity * 100
        )

        for idx, row in profile.iterrows():
            if row["Adjustment_Records"] < 5 or pd.isna(row["Efficiency_vs_Baseline_%"]):
                continue
...
with col_f2:
    available_positions = sorted(
        master_df.loc[master_df["Resin"] == selected_resin, "Position_UI"].dropna().unique()
    )
...
with tab1:
    ...
    c1.metric("Valid Paint Batches (n ≥ 30)", f"{unique_batch_count:,}")
    c2.metric("Valid Adjustment Records", f"{adjustment_record_count:,}")
    ...
    c5.metric("Max Drop (Delta V)", f"{system_df['Delta_V'].max():.1f} s")
    ...
    fig_scatter.add_trace(go.Scatter(
        ...
        customdata=plot_df[["黏度(秒)_1", "Delta_V", "Initial_Viscosity_Zone", "塗料批號"]].values,
        hovertemplate=(
            "<b>Batch: %{customdata[3]}</b><br><b>Zone: %{customdata[2]}</b><br>"
            "Solvent Ratio: %{x:.2f}%<br>Initial Visc (Before): %{y:.1f}s<br>"
    ...
    fig_scatter.add_trace(go.Scatter(
        ...
        customdata=plot_df[["黏度(秒)", "Delta_V", "Initial_Viscosity_Zone", "塗料批號"]].values,
        hovertemplate=(
            "<b>Batch: %{customdata[3]}</b><br><b>Zone: %{customdata[2]}</b><br>"
            "Solvent Ratio: %{x:.2f}%<br>Initial Visc (Before): %{customdata[0]:.1f}s<br>"
    ...
    fig_scatter.update_layout(
        title=dict(text=chart_title, x=0.5, xanchor="center", y=0.97, yanchor="top", font=dict(size=18, color="#1F3855")),
        plot_bgcolor="white",
        paper_bgcolor="white",
        height=620,
...
with tab2:
    ...
    with col_i1:
        curr_visc = st.number_input("Current Viscosity (s)", value=st.session_state["exec_curr_visc"], step=1.0, key="iv")
    with col_i2:
        app_lsl = st.number_input("Approved Target LSL (s)", value=st.session_state["exec_lsl"], step=1.0, key="ilsl")
    with col_i3:
        app_usl = st.number_input("Approved Target USL (s)", value=st.session_state["exec_usl"], step=1.0, key="iusl")
    with col_i4:
        order_weight = st.number_input("Order Paint Weight (kg)", value=st.session_state["exec_order_weight"], step=10.0, key="iow")
    ...
    if st.button("🚀 Calculate Optimized SOP", type="primary"):
        if curr_visc <= 0 or app_lsl <= 0 or app_usl <= 0 or order_weight <= 0:
            ...
            if required_drop <= 0:
                ...
                if record_count < 5:
                    st.error("🚨 SYSTEM BLOCKED: Insufficient historical data (<5 records). Manual Process Engineer verification required.")
                    st.session_state["calculation_done"] = False
                else:
                    ref_sensitivity = ref_data["Sensitivity"].median()
                    ref_ratio_p90 = ref_data["Solvent_Ratio_Percent"].quantile(0.90)
                    ref_ratio_p95 = ref_data["Solvent_Ratio_Percent"].quantile(0.95)
                    ref_drop_p90 = ref_data["Delta_V"].quantile(0.90)
                    ref_drop_max = ref_data["Delta_V"].max()

                    required_ratio = required_drop / ref_sensitivity
                    recommended_solvent = order_weight * (required_ratio / 100)
                    max_total_solvent = order_weight * (ref_ratio_p90 / 100)
                    ...
                    if not pd.isna(saturation_limit_ratio) and required_ratio >= saturation_limit_ratio:
                        risk_status = "🚨 SATURATION LIMIT REACHED: Historical data shows very low dilution efficiency in this range."
                        risk_color = "red"
                        blocked = True
                        saturation_note = f"Required ratio: {required_ratio:.2f}% | Historical saturation limit: {saturation_limit_ratio:.2f}%"
                    elif not blocked and not pd.isna(saturation_warning_ratio) and required_ratio >= saturation_warning_ratio:
                        risk_status = "⚠️ DIMINISHING RETURNS WARNING: Historical dilution efficiency has started declining."
                        risk_color = "orange"
                        saturation_note = f"Required ratio: {required_ratio:.2f}% | Diminishing-return threshold: {saturation_warning_ratio:.2f}%"

                    st.markdown(
                        f"### Assessment: <span style='color:{risk_color}'>{risk_status}</span>",
                        unsafe_allow_html=True
                    )

                    if saturation_note:
                        st.warning(f"📉 Saturation screening: {saturation_note}")
                    ...
                    if blocked:
                        ...
                        st.success(
                            f"**Calculated Total Recommendation:** `{recommended_solvent:.2f} kg` "
                            f"(Absolute Limit: {max_total_solvent:.2f} kg)"
                        )

                        st.markdown("### 🛠️ Execution Protocol (3-Stage Addition)")
                        st.markdown(
                            f"* **Step 1 (60%):** Add `{recommended_solvent * 0.60:.2f} kg` → Agitate & Measure.\n"
                            f"* **Step 2 (25%):** Add `{recommended_solvent * 0.25:.2f} kg` → Only if > USL.\n"
                            f"* **Step 3 (15%):** Add up to `{recommended_solvent * 0.15:.2f} kg` → Fine Micro-Adjustment."
                        )
...
with tab3:
    ...
    st.markdown("Full operational baseline matrix for safety boundary definitions. Filtered by selected Resin, Position & Vendor.")

    eng_matrix = system_df.groupby("Initial_Viscosity_Zone", observed=False).agg(
        Adjustment_Records=("塗料批號", "size"),
        Paint_Batches=("塗料批號", "nunique"),
        Sensitivity_Median=("Sensitivity", "median"),
        Ratio_Median=("Solvent_Ratio_Percent", "median"),
    ...
    if pd.isna(baseline_sensitivity):
        st.warning("⚠️ Insufficient zone-level data for saturation analysis. Each ratio zone needs at least 5 valid adjustment records.")
    else:
        col_sat1, col_sat2, col_sat3 = st.columns(3)
        col_sat1.metric("Baseline Sensitivity", f"{baseline_sensitivity:.2f} s/%")
        col_sat2.metric("Diminishing Returns Threshold", f"{saturation_warning_ratio:.2f}%" if not pd.isna(saturation_warning_ratio) else "Not Detected")
        col_sat3.metric("Saturation Limit", f"{saturation_limit_ratio:.2f}%" if not pd.isna(saturation_limit_ratio) else "Not Detected")
        ...
        st.dataframe(
            ...
            column_config={
                ...
                "Paint_Batches": st.column_config.NumberColumn("涉及塗料批號數", format="%d"),
                "Ratio_Median": st.column_config.NumberColumn("Median Ratio (%)", format="%.2f"),
                "Ratio_Min": st.column_config.NumberColumn("Min Ratio (%)", format="%.2f"),
                "Ratio_Max": st.column_config.NumberColumn("Max Ratio (%)", format="%.2f"),
                "DeltaV_Median": st.column_config.NumberColumn("Median Delta V (s)", format="%.2f"),
                "Sensitivity_Median": st.column_config.NumberColumn("Median Sensitivity (s/%)", format="%.2f"),
...
with tab4:
    ...
    def create_worker_viscosity_zone(df):
        ...
        temp_df["Worker_Viscosity_Zone"] = temp_df["黏度(秒)"].apply(base_zone)
    ...
    def format_actual_range(p25, p75):
        ...
        return f"{p25:.1f}" if abs(p25 - p75) < 0.05 else f"{p25:.1f} - {p75:.1f}"

    matrix_df = create_worker_viscosity_zone(matrix_df)

    worker_sop = matrix_df.groupby(
        ["Resin", "Position_UI", "Vendor", "Solvent_Type", "Worker_Viscosity_Zone"],
        observed=False
    ).agg(
        Adjustment_Records=("塗料批號", "size"),
    ...
    worker_sop["Final_Visc_P25_P75"] = worker_sop.apply(
        lambda row: format_actual_range(row["Final_Visc_P25"], row["Final_Visc_P75"]),
        axis=1
    )

    saturation_summary = []
    system_keys = matrix_df[["Resin", "Position_UI", "Vendor", "Solvent_Type"]].drop_duplicates()
    ...
    worker_sop = worker_sop.merge(
        pd.DataFrame(saturation_summary),
        on=["Resin", "Position_UI", "Vendor", "Solvent_Type"],
        how="left"
    )

    worker_sop["Saturation_Warning_Ratio"] = worker_sop["Saturation_Warning_Ratio"].fillna(worker_sop["Ratio_P90"])
    worker_sop["Saturation_Stop_Ratio"] = worker_sop["Saturation_Stop_Ratio"].fillna(worker_sop["Ratio_P95"])
    ...
    worker_output = worker_sop[
        [
            "Resin", "塗裝位置", "Vendor", "Solvent_Type", "Worker_Viscosity_Zone",
            "Adjustment_Records", "History_Batches", "Ref_Start_Visc",
            "Ref_Paint_Weight_kg", "Ref_Solvent_Add_kg", "Ref_Solvent_Ratio",
            "Final_Visc_P25_P75", "Saturation_Warning_Ratio", "Saturation_Stop_Ratio"
        ]
    ].copy()
    ...
    def get_zone_order(zone):
        ...
        if zone.startswith("<=70"):
            return 1
        if zone.startswith("71-90"):
            return 2
        if zone.startswith("91-110"):
            return 3
        if zone.startswith("111-130"):
            return 4
        if zone.startswith("130-"):
            return 5
        return 99

    worker_output["_zone_order"] = worker_output["初始黏度區間"].apply(get_zone_order)
    worker_output = worker_output.sort_values(
        by=["樹脂種類", "塗裝位置", "塗料供應商", "稀釋劑種類", "_zone_order"]
    ...
    st.dataframe(
        ...
        column_config={
            ...
            "有效調整紀錄數": st.column_config.NumberColumn("有效調整紀錄數", format="%d"),
            "涉及塗料批號數": st.column_config.NumberColumn("涉及塗料批號數", format="%d"),
            "參考起始黏度": st.column_config.NumberColumn("參考起始黏度 (秒)", format="%.1f"),
            "參考塗料使用量": st.column_config.NumberColumn("參考塗料使用量 (kg)", format="%.1f"),
            "參考稀釋劑添加量": st.column_config.NumberColumn("參考稀釋劑添加量 (kg)", format="%.1f"),
            "參考稀釋劑添加比例": st.column_config.NumberColumn("參考稀釋劑添加比例 (%)", format="%.2f"),
            "歷史最終黏度範圍": st.column_config.TextColumn("歷史最終黏度範圍 (秒, P25–P75)"),
            "飽和警戒比例": st.column_config.NumberColumn("飽和警戒比例 (%)", format="%.2f"),
            "飽和停止比例": st.column_config.NumberColumn("飽和停止比例 (%)", format="%.2f")
        },
        use_container_width=True,
        hide_index=True
    )

    st.caption("「有效調整紀錄數」為實際用於計算中位數、P90/P95的調整資料筆數；「涉及塗料批號數」為這些紀錄涵蓋的不同塗料批號數。")
    st.caption("「歷史最終黏度範圍」為相同條件下歷史最終黏度的中間50%分布，僅供現場調整參考，非產品規格上下限。")
    st.caption("「130–Max」表示該樹脂、塗裝位置、供應商及稀釋劑組合中，歷史實際出現的最高初始黏度範圍；飽和停止比例仍以歷史效率分析或P95作為安全限制。")

    csv_export = worker_output.to_csv(index=False).encode("utf-8-sig")
    st.download_button(
