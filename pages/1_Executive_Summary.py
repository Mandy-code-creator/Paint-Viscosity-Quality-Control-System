...
FIRST_ADD_PERCENT = 0.60         # First addition standard fallback
...
with tab2:
    ...
    st.info(
        ...
        "1. 第一次添加將被嚴格限制在警戒比例的 50% 或 ngưỡng an toàn tuyệt đối.\n"
    ...
    if st.button("🚀 Calculate First Addition", type="primary", use_container_width=True):
        ...
        if current_visc <= 0 or target_lsl <= 0 or target_usl <= 0 or actual_paint_weight <= 0:
            ...
            if len(reference_df) < MIN_REFERENCE_RECORDS:
                ...
                first_add_ratio = min(warning_ratio * 0.5, max_safe_ratio)
                first_add_kg = (actual_paint_weight * first_add_ratio) / 100

                blocked = False
                if first_add_ratio <= 0:
                    blocked = True
                    st.error("🚨 安全鎖定觸發：系統計算首次安全添加量為 0，不可執行自動加料。")
                else:
                    risk_status = "✅ 系統已啟動雙重安全鎖定，請執行首次添加。"
                    risk_color = "green"
                ...
                if not blocked:
                    ...
                    result_col1.metric("Required Viscosity Drop", f"{current_visc - target_center:.1f} s")
                    ...
                    st.markdown(
                        ...
                        f"2. 第一次安全添加比例：`{first_add_ratio:.2f}%`\n"
                        f"3. 第一次安全添加量：`{first_add_kg:.2f} kg`\n"
                        f"4. 添加後攪拌至少：`{MIXING_TIME_MINUTES} 分鐘`\n"
                        f"5. 攪拌後重新量測黏度，再使用下方功能計算下一步。"
                    )
        ...
        if sop_result:
            ...
            if st.button("Calculate Additional Solvent", type="secondary", use_container_width=True):
                if actual_step1_kg <= 0 or measured_after_step1 <= 0:
                    ...
                    E_actual = (sop_result["initial_visc"] - measured_after_step1) / actual_step1_ratio
                    R_required = (measured_after_step1 - sop_result["target_center"]) / E_actual if E_actual > 0 else 0

                    # CLOSED LOOP STEP 3: Three-Layer Protection
                    remaining_to_warning = sop_result["warning_ratio"] - actual_step1_ratio
                    remaining_to_stop = sop_result["stop_ratio"] - actual_step1_ratio

                    if remaining_to_warning > 0:
                        limit_2 = STEP_MAX_RATIO_NORMAL
                        limit_3 = remaining_to_warning
                        warning_triggered = False
                    else:
                        limit_2 = STEP_MAX_RATIO_MICRO
                        limit_3 = remaining_to_stop
                        warning_triggered = True

                    additional_ratio = min(R_required * K_FACTOR, limit_2, max(0.0, limit_3))
                    ...
                    if remaining_to_stop <= 0:
                        st.error("🚨 已達停止比例。不可繼續添加稀釋劑。")
                    elif additional_ratio <= 0:
                        st.error("🚨 剩餘空間不足以繼續安全添加。請通知製程工程師。")
                    else:
                        if warning_triggered:
                            st.warning("⚠️ 已超過警戒比例。系統已切換為微調模式 (Micro-stepping)，請嚴格遵守單次最高添加限制。")
                        
                        step2_col1, step2_col2, step2_col3, step2_col4 = st.columns(4)
                        step2_col1.metric("Actual Efficiency", f"{E_actual:.2f} s/%")
                        step2_col2.metric("Theoretical Required", f"{R_required:.2f}%")
                        step2_col3.metric("Safe Step Ratio", f"{additional_ratio:.2f}%")
                        step2_col4.metric("Additional Solvent", f"{additional_kg:.2f} kg")

                        st.success(f"### 建議下一次添加：{additional_kg:.2f} kg")
                        st.markdown(
                            "**重要：** 系統已應用衰減係數與步進限制。添加後請務必再次量測。\n\n"
                            f"目前累積比例將達: `{total_ratio_after_step2:.2f}%` (停止線: `{sop_result['stop_ratio']:.2f}%`)"
                        )
...
with tab4:
    ...
    st.warning("操作順序：查詢相同條件 → 依建議首次添加比例加料 (已啟用雙重安全鎖定) → 攪拌5分鐘 → 重新量測。累積添加比例不得超過停止比例。")
    st.caption("註：稀釋劑比例以實際塗料重量為基準。最終黏度範圍已套用 SPC (1-Sigma) 進行優化以排除異常值。")
    ...
    group_cols_worker = ["Resin", "Position_UI", "Vendor", "Solvent_Type", "Worker_Viscosity_Zone"]

    worker_sop = (
        matrix_df.groupby(group_cols_worker, observed=False)
        .agg(
            Adjustment_Records=("塗料批號", "size"),
            ...
            First_Add_Ratio=("Solvent_Ratio_Percent", lambda x: x.median() * FIRST_ADD_PERCENT),
    ...
    saturation_summary = []
    system_keys = matrix_df[group_cols_worker].drop_duplicates()
    for _, system_row in system_keys.iterrows():
        temp_zone_df = matrix_df[
            (matrix_df["Resin"] == system_row["Resin"]) & (matrix_df["Position_UI"] == system_row["Position_UI"]) &
            (matrix_df["Vendor"] == system_row["Vendor"]) & (matrix_df["Solvent_Type"] == system_row["Solvent_Type"]) &
            (matrix_df["Worker_Viscosity_Zone"] == system_row["Worker_Viscosity_Zone"])
        ].copy()
        temp_saturation = build_saturation_profile(temp_zone_df)
        temp_limits = get_safety_limits(temp_zone_df, temp_saturation)
        saturation_summary.append({
            "Resin": system_row["Resin"], "Position_UI": system_row["Position_UI"],
            "Vendor": system_row["Vendor"], "Solvent_Type": system_row["Solvent_Type"],
    ...
    worker_sop = worker_sop.merge(saturation_summary_df, on=group_cols_worker, how="left")
    ...
    # Enforce Dual-Safety Lock on Tab 4 First_Add_Ratio (Cap at 50% of Warning)
    worker_sop["First_Add_Ratio"] = np.minimum(worker_sop["First_Add_Ratio"], worker_sop["Saturation_Warning_Ratio"] * 0.5)
    ...
    worker_output = worker_sop[[
        ...
        "Adjustment_Records", "History_Batches", "Ref_Start_Visc", "Ref_Sensitivity",
        "Historical_Total_Ratio", "First_Add_Ratio", "Historical_Final_Visc_Range",
        "Historical_Temp_Range", "Saturation_Warning_Ratio", "Saturation_Stop_Ratio"
    ]].copy()

    worker_output = worker_output.rename(columns={
        "Resin": "樹脂種類", "Vendor": "塗料供應商", "Solvent_Type": "稀釋劑種類",
        "Worker_Viscosity_Zone": "初始黏度區間", "Adjustment_Records": "有效調整紀錄數",
        "History_Batches": "涉及塗料批號數", "Ref_Start_Visc": "參考起始黏度",
        "Ref_Sensitivity": "參考稀釋效率", "Historical_Total_Ratio": "歷史總添加比例",
        "First_Add_Ratio": "建議首次添加比例", "Historical_Final_Visc_Range": "歷史最終黏度範圍",
        "Historical_Temp_Range": "歷史參考溫度範圍", "Saturation_Warning_Ratio": "累積添加警戒比例",
        "Saturation_Stop_Ratio": "累積添加停止比例"
    })

    worker_output["_zone_order"] = worker_output["初始黏度區間"].apply(get_zone_order)
    worker_output = worker_output.sort_values(by=["樹脂種類", "塗裝位置", "塗料供應商", "稀釋劑種類", "_zone_order"]).drop(columns="_zone_order")
    ...
    st.dataframe(
        ...
        column_config={
            ...
            "歷史總添加比例": st.column_config.NumberColumn("歷史總添加比例 (%)", format="%.2f"),
            "建議首次添加比例": st.column_config.NumberColumn("建議首次添加比例 (%)", format="%.2f"),
            "歷史最終黏度範圍": st.column_config.TextColumn("歷史最終黏度範圍 (1-Sigma)"),
            "歷史參考溫度範圍": st.column_config.TextColumn("歷史參考溫度範圍 (°C, P25-P75)"),
    ...
    st.markdown(
        ...
        "3. 確認參考稀釋效率，評估該塗料的敏感度。\n"
        "4. 輸入或秤量實際塗料重量。\n"
        "5. 第一次添加量 = 實際塗料重量 × 建議首次添加比例 ÷ 100。\n"
        "6. 攪拌至少 5 分鐘後重新量測黏度。\n"
        "7. 若仍高於 USL，回到 Tab 2 依實測結果與閉迴路控制邏輯計算下一次添加量。\n"
        "8. 累積添加比例不得超過「累積添加停止比例」。"
    )

    csv_export = worker_output.to_csv(index=False).encode("utf-8-sig")
    st.download_button(label="下載現場歷史加料參考表 CSV", data=csv_export, file_name="現場歷史加料參考表.csv", mime="text/csv")
