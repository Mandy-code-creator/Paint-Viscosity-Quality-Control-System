...
# ==========================================
# 1. Clean data to ensure valid calculations
if all(col in group_a.columns for col in [visc_before, visc_after, paint_weight, solvent_weight]):
    ...
    df_clean = df_clean[
        ...
        (df_clean[visc_before] != df_clean[visc_after]) # Must have some effect
    ]
...
# Define fixed color map for consistency across charts using a professional qualitative palette
    ...
    for j in range(2):
        if i + j < len(resins):
            ...
            with cols[j]:
                ...
                # --- B. CONFIGURE PROFESSIONAL LAYOUT & TEXT (No Shadows/Halos) ---
                fig_bar.update_layout(
                    plot_bgcolor='white', 
                    height=300, 
                    # Set clear, dark font with no shadow effects
