...
with tab_pilot:
    ...
    st.markdown("依據塗料使用量、稀釋劑用量、添加比例穩定性及歷史資料量，評估適合優先試用預調漆的色號。")

    # 1. UI Settings
    set1, set2, set3, set4 = st.columns(4)
    min_pilot_records = set1.number_input("最低歷史紀錄數 (Min Records)", min_value=5, value=20, step=1)
    relative_iqr_limit = set2.number_input("相對四分位距上限 (Max Rel. IQR)", min_value=0.05, max_value=1.00, value=0.25, step=0.05)
    stable_coverage_limit = set3.number_input("穩定區間涵蓋率下限 (Min Coverage)", min_value=0.30, max_value=1.00, value=0.70, step=0.05)
    top_n_pilot = set4.number_input("顯示色號數 (Top N)", min_value=3, max_value=30, value=10, step=1)

    st.caption("穩定性判定以相對四分位距及穩定區間涵蓋率為主。穩定區間定義為各色號添加比例中位數 ±2 個百分點；CV僅供工程參考。")

    # 2. Optimized Aggregation Helper
    def calc_coverage(s):
        s_clean = pd.to_numeric(s, errors="coerce").dropna()
        if s_clean.empty: return np.nan
        med = s_clean.median()
        return s_clean.between(med - 2.0, med + 2.0, inclusive="both").mean()
    ...
    pilot_df = filter_df.groupby(["Vendor", "Resin", "Position_UI", "Solvent_Type", "Paint_Code"], dropna=False).agg(
        ...
        Total_Solvent_kg=("添加重量", "sum"),
        Median_Ratio_Percent=("Solvent_Ratio_Percent", "median"),
        Ratio_P25=("Solvent_Ratio_Percent", lambda x: x.quantile(0.25)),
        Ratio_P75=("Solvent_Ratio_Percent", lambda x: x.quantile(0.75)),
        Stable_Coverage=("Solvent_Ratio_Percent", calc_coverage),
        Ratio_CV=("Solvent_Ratio_Percent", lambda x: x.std(ddof=1)/x.mean() if x.mean() > 0 else np.nan),
        Median_Before_Viscosity=("黏度(秒)", "median"),
        Median_After_Viscosity=("黏度(秒)_1", "median"),
        After_Viscosity_P25=("黏度(秒)_1", lambda x: x.quantile(0.25)),
        After_Viscosity_P75=("黏度(秒)_1", lambda x: x.quantile(0.75)),
        Median_Viscosity_Drop=("Delta_V", "median"),
        Production_Lines=("線別", lambda x: x[x != "Unknown"].nunique())
    ).reset_index()

    # Math & Metrics Computation
    ...
    pilot_df["Relative_IQR"] = np.where(pilot_df["Median_Ratio_Percent"] > 0, (pilot_df["Ratio_P75"] - pilot_df["Ratio_P25"]) / pilot_df["Median_Ratio_Percent"], np.nan)
    
    # Vectorized MinMax Scaling Function
    def v_minmax(series):
        s_min, s_max = series.min(), series.max()
        return pd.Series(100.0, index=series.index) if s_max == s_min else ((series - s_min) / (s_max - s_min) * 100).fillna(0).clip(0, 100)

    # 4. Scoring Engine
    score_usage = v_minmax(pilot_df["Total_Solvent_kg"]) * 0.30
    score_paint = v_minmax(pilot_df["Total_Paint_kg"]) * 0.20
    score_ratio = v_minmax(pilot_df["Weighted_Ratio_Percent"]) * 0.20
    score_stab  = ((100 - v_minmax(pilot_df["Relative_IQR"])) * 0.50 + pilot_df["Stable_Coverage"].fillna(0).clip(0, 1) * 50) * 0.20
    score_data  = v_minmax(pilot_df["Historical_Records"]) * 0.10

    pilot_df["Pilot_Score"] = score_usage + score_paint + score_ratio + score_stab + score_data
    pilot_df["Data_Qualified"] = pilot_df["Historical_Records"] >= min_pilot_records
    pilot_df["Robust_Stable"] = (pilot_df["Relative_IQR"] <= relative_iqr_limit) & (pilot_df["Stable_Coverage"] >= stable_coverage_limit)

    # Vectorized Classification
    conditions = [
        ~pilot_df["Data_Qualified"],
        (pilot_df["Pilot_Score"] >= 70) & pilot_df["Robust_Stable"],
        (pilot_df["Pilot_Score"] >= 45)
    ]
    pilot_df["Evaluation_Result"] = np.select(conditions, ["暫不建議", "優先試用", "可進一步評估"], default="暫不建議")
    
    pilot_df = pilot_df.sort_values(["Pilot_Score", "Total_Solvent_kg"], ascending=[False, False]).reset_index(drop=True)
    pilot_df.insert(0, "Pilot_Rank", np.arange(1, len(pilot_df) + 1))
    pilot_top_df = pilot_df.head(int(top_n_pilot)).copy()

    # 5. KPI Summary
    kpi1, kpi2, kpi3, kpi4 = st.columns(4)
    kpi1.metric("Evaluated Codes", f"{len(pilot_df):,}")
    kpi2.metric("Data-Qualified", f"{pilot_df['Data_Qualified'].sum():,}")
    kpi3.metric("Priority Pilot", f"{(pilot_df['Evaluation_Result'] == '優先試用').sum():,}")
    kpi4.metric("Top Candidate", pilot_df.iloc[0]["Paint_Code"] if not pilot_df.empty else "-")

    # 6. Chart 1: Priority Ranking
    color_map = {"優先試用": "#5AD8A6", "可進一步評估": "#F6BD16", "暫不建議": "#E8684A"}
    
    fig_pilot = px.bar(
        pilot_top_df.sort_values("Pilot_Score"), x="Pilot_Score", y="Paint_Code", orientation="h",
        color="Evaluation_Result", color_discrete_map=color_map, text_auto=".1f",
        custom_data=["Total_Solvent_kg", "Total_Paint_kg", "Relative_IQR"],
        title=f"預調漆試用色號優先順序<br><sup>Filters: {filter_details}</sup>",
        height=max(500, len(pilot_top_df) * 45)
    )
    fig_pilot.update_traces(
        textposition="outside", 
        hovertemplate="<b>%{y}</b><br>Score: %{x:.1f}<br>Solvent: %{customdata[0]:,.1f}kg<br>Paint: %{customdata[1]:,.1f}kg<br>Rel IQR: %{customdata[2]:.3f}<extra></extra>"
    )
    fig_pilot.update_layout(plot_bgcolor="white", xaxis=dict(range=[0, 110], showgrid=True, gridcolor="#EAEAEA"), legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="right", x=1))
    st.plotly_chart(fig_pilot, use_container_width=True)
    exported_figs["9. Pilot Priority Ranking"] = fig_pilot

    # 7. Chart 2: Robust Stability vs Opportunity (Optimized Scatter)
    max_paint = pilot_top_df["Total_Paint_kg"].max()
    pilot_top_df["Bubble_Size"] = 24 if pd.isna(max_paint) or max_paint <= 0 else np.sqrt(pilot_top_df["Total_Paint_kg"] / max_paint) * 42 + 16

    fig_opp = px.scatter(
        pilot_top_df, x="Relative_IQR", y="Total_Solvent_kg", size="Bubble_Size", color="Evaluation_Result",
        color_discrete_map=color_map, text="Paint_Code", hover_name="Paint_Code",
        title="各色號稀釋劑使用量與添加比例穩健性", height=600
    )
    fig_opp.update_traces(textposition="top right", marker=dict(opacity=0.8, line=dict(width=1, color="white")))
    fig_opp.add_vline(x=relative_iqr_limit, line_dash="dash", line_color="black", annotation_text=f"Limit={relative_iqr_limit:.2f}")
    fig_opp.update_layout(plot_bgcolor="white", xaxis=dict(title="相對四分位距 (Rel IQR)", rangemode="tozero", showgrid=True, gridcolor="#EAEAEA"), yaxis=dict(title="稀釋劑總用量 (Solvent kg)", rangemode="tozero", showgrid=True, gridcolor="#EAEAEA"), legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="right", x=1))
    
    st.plotly_chart(fig_opp, use_container_width=True)
    exported_figs["10. Solvent Opportunity vs Stability"] = fig_opp

    # 8. Data Table & Conclusion
    st.markdown("---")
    st.markdown("### 評估結論 (Conclusion)")
    
    if not pilot_df.empty:
        top = pilot_df.iloc[0]
        st.success(f"**Top Candidate:** {top['Paint_Code']} (Score: {top['Pilot_Score']:.1f}) ➔ **{top['Evaluation_Result']}**")
        st.markdown(f"- Paint Used: **{top['Total_Paint_kg']:,.1f} kg** | Solvent Used: **{top['Total_Solvent_kg']:,.1f} kg** | Weighted Ratio: **{top['Weighted_Ratio_Percent']:.2f}%**")
        st.markdown(f"- Rel IQR: **{top['Relative_IQR']:.3f}** | Coverage: **{top['Stable_Coverage']:.1%}** | Records: **{top['Historical_Records']}**")
    
    # Render Output DataFrame mapping
    display_cols = {
        "Pilot_Rank": "排名 (Rank)", "Paint_Code": "色號 (Code)", "Evaluation_Result": "評估結果 (Result)",
        "Pilot_Score": "分數 (Score)", "Historical_Records": "紀錄數 (Records)", 
        "Total_Paint_kg": "塗料用量 (Paint kg)", "Total_Solvent_kg": "稀釋劑用量 (Solvent kg)",
        "Weighted_Ratio_Percent": "加權比例 (Wt. Ratio %)", "Relative_IQR": "相對 IQR", "Stable_Coverage": "涵蓋率 (Coverage)"
    }
    st.dataframe(pilot_df[list(display_cols.keys())].rename(columns=display_cols), use_container_width=True, hide_index=True)
    
    st.download_button("📥 下載評估表 (Download CSV)", pilot_df.to_csv(index=False).encode("utf-8-sig"), "Trial_Evaluation.csv", "text/csv")
