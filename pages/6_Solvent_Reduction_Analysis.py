...
# ==========================================
# 7. XUẤT BÁO CÁO WORD (EXPORT TO DOCX)
# ==========================================
import gc
...
if not HAS_DOCX:
    ...
    # Nút bấm xuất file Word
    if st.button("📥 Generate & Download Word Report", type="primary"):
        with st.spinner("⏳ Đang tạo file Word và xử lý biểu đồ (vui lòng đợi vài giây)..."):
            try:
                # 1. Tạo document mới
                doc = Document()
                doc.add_heading('Báo Cáo Phân Tích Giảm Lượng Dung Môi (Solvent Reduction Analysis)', 0)
                ...
                # 2. Lặp qua các biểu đồ đã lưu trong exported_figs
                for fig_title, fig in exported_figs.items():
                    doc.add_heading(fig_title, level=2)
                    
                    # Chuyển đổi Plotly figure sang ảnh PNG bằng kaleido 
                    # (Lưu ý: Phải dùng kaleido==0.1.0.post1 để tránh trap app)
                    img_bytes = fig.to_image(format="png", width=800, height=500, scale=2)
                    
                    # Đọc byte ảnh vào buffer
                    image_stream = io.BytesIO(img_bytes)
                    
                    # Chèn vào Word
                    doc.add_picture(image_stream, width=Inches(6.0))
                    
                    # Giải phóng bộ nhớ ngay lập tức để tránh tràn RAM
                    del img_bytes
                    del image_stream
                    gc.collect() 
                
                # 3. Lưu file Word vào buffer để tải về
                ...
                # 4. Hiển thị nút tải xuống
                st.download_button(
                    label="💾 Tải file Word (.docx)",
                    data=doc_buffer,
                    file_name=f"Solvent_Analysis_Report_{pd.Timestamp.now().strftime('%Y%m%d')}.docx",
                    mime="application/vnd.openxmlformats-officedocument.wordprocessingml.document"
                )
                
            except Exception as e:
                st.error(f"❌ Có lỗi xảy ra trong quá trình xuất ảnh. Gợi ý: Hãy đảm bảo bạn đã cài `kaleido==0.1.0.post1`.\\n\\nChi tiết lỗi: {e}")
