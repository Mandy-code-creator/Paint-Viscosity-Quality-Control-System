...
st.caption(
    "Only paint codes with at least 730 days of historical data coverage "
    "under the selected filters are included."
)
...
def build_paint_code_summary(source_df):
    ...
    if "eligible_coverage" in globals():
...
# =========================================================
# 11. KEEP PAINT CODES WITH AT LEAST 2 YEARS OF DATA
# =========================================================
MIN_COVERAGE_DAYS = 730

if filter_df["_Analysis_Date"].notna().any():
    paint_code_coverage = filter_df.groupby(
        "Paint_Code",
        observed=False,
        dropna=False,
    ).agg(
        First_Record_Date=("_Analysis_Date", "min"),
        Last_Record_Date=("_Analysis_Date", "max"),
        Coverage_Records=("Paint_Code", "size"),
    ).reset_index()

    paint_code_coverage["Coverage_Days"] = (
        paint_code_coverage["Last_Record_Date"]
        - paint_code_coverage["First_Record_Date"]
    ).dt.days

    paint_code_coverage["Coverage_Months"] = (
        paint_code_coverage["Coverage_Days"] / 30.4375
    )

    eligible_paint_codes = paint_code_coverage.loc[
        paint_code_coverage["Coverage_Days"] >= MIN_COVERAGE_DAYS,
        "Paint_Code",
    ].tolist()

    filter_df = filter_df[
        filter_df["Paint_Code"].isin(eligible_paint_codes)
    ].copy()

    if filter_df.empty:
        st.warning(
            "⚠️ No paint code has at least 2 years of historical data "
            "under the selected global filters."
        )
        st.stop()

    eligible_coverage = paint_code_coverage[
        paint_code_coverage["Paint_Code"].isin(eligible_paint_codes)
    ].copy()

else:
    st.warning(
        "⚠️ A valid date column is required to apply the minimum "
        "2-year data-coverage condition."
    )
    st.stop()
...
# =========================================================
with tab_ranking:
    st.subheader("1. Paint Code Solvent Consumption Analysis")

    st.caption(
        f"Analysis Period: {period_label} | "
        f"Records Included: {len(filter_df):,} | "
        f"Eligibility Rule: Data coverage ≥ 730 days"
    )
    ...
    if summary_df.empty:
        ...
        # -----------------------------------------------------
        # CHART 1:
        # Total paint and total solvent on the same chart
        # -----------------------------------------------------
        st.markdown("#### Paint and Solvent Consumption by Paint Code")
        ...
        chart_height = max(5.5, len(chart_df) * 0.52)

        fig1, ax_paint = plt.subplots(
            figsize=(12, chart_height),
            dpi=160,
        )

        y_positions = np.arange(len(chart_df))
        bar_height = 0.34

        paint_bars = ax_paint.barh(
            y_positions + bar_height / 2,
            chart_df["Total_Paint_kg"],
            height=bar_height,
            label="Total Paint Usage",
            alpha=0.78,
        )

        ax_solvent = ax_paint.twiny()

        solvent_bars = ax_solvent.barh(
            y_positions - bar_height / 2,
            chart_df["Total_Solvent_kg"],
            height=bar_height,
            label="Total Solvent Usage",
            alpha=0.82,
        )
        ...
        ax_paint.set_yticks(y_positions)
        ax_paint.set_yticklabels(y_labels)

        ax_paint.set_xlabel("Total Paint Usage (kg)")
        ax_solvent.set_xlabel("Total Solvent Usage (kg)")
        ax_paint.set_ylabel("Paint Code")

        ax_paint.set_title(
            "Total Paint and Solvent Usage by Paint Code",
            loc="left",
            fontweight="bold",
            pad=14,
        )
        ...
        handles1, labels1 = ax_paint.get_legend_handles_labels()
        handles2, labels2 = ax_solvent.get_legend_handles_labels()
        ...
        for spine in ax_paint.spines.values():
            spine.set_visible(True)
            spine.set_linewidth(1.0)

        for spine in ax_solvent.spines.values():
            spine.set_visible(True)
            spine.set_linewidth(1.0)
        ...
        # -----------------------------------------------------
        # CHART 2:
        # Weighted solvent ratio comparison
        # -----------------------------------------------------
        st.markdown("#### Weighted Solvent Ratio by Paint Code")

        ratio_df = summary_df.sort_values(
            "Weighted_Ratio_Percent",
            ascending=True,
        ).copy()

        fig2, ax2 = plt.subplots(
            figsize=(12, max(5.5, len(ratio_df) * 0.50)),
            dpi=160,
        )
        ...
        for bar, ratio, solvent_kg in zip(
            ratio_bars,
            ratio_df["Weighted_Ratio_Percent"],
            ratio_df["Total_Solvent_kg"],
        ):
            ax2.text(
                bar.get_width(),
                bar.get_y() + bar.get_height() / 2,
                f" {ratio:.2f}% | {solvent_kg:,.0f} kg solvent",
                va="center",
                fontsize=8,
            )
        ...
        ax2.set_xlabel("Weighted Solvent Ratio (%)")
        ax2.set_ylabel("Paint Code")
        ...
        # SUMMARY TABLE
        # -----------------------------------------------------
        st.markdown("#### Paint Code Summary")
        ...
        for col in ["First Record Date", "Last Record Date"]:
        ...
        st.download_button(
            label="📥 Download Paint Code Summary",
            data=csv_data,
            file_name="Paint_Code_Solvent_Consumption.csv",
            mime="text/csv",
