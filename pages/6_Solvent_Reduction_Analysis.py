...
with tab_pilot:
    ...
    # ------------------------------------------------------
    # Evaluation chart 2: 添加比例穩定率
    # Simple horizontal bar chart for management review.
    # ------------------------------------------------------
    stability_chart_df = (
        pilot_top_df
        .sort_values("Stable_Coverage", ascending=True)
        .copy()
    )

    stability_chart_df["Stable_Coverage_Percent"] = (
        stability_chart_df["Stable_Coverage"] * 100
    )

    stability_chart_df["Bar_Label"] = stability_chart_df.apply(
        lambda row: (
            f"{row['Stable_Coverage_Percent']:.0f}%｜"
            f"{row['Stability_Level']}｜"
    ...
    fig_stability = px.bar(
        ...
        title=(
            "各色號添加比例穩定率"
            "<br><sup>穩定率＝落在添加比例中位數 ±2 個百分點內的紀錄占比；"
            "數值越高代表比例越穩定</sup>"
        ),
        height=max(500, len(stability_chart_df) * 48)
    )
    ...
    fig_stability.update_traces(
        ...
        hovertemplate=(
            ...
            "添加比例穩定率：%{x:.1f}%<br>"
            "穩定度：%{customdata[3]}<br>"
            "添加比例中位數：%{customdata[0]:.2f}%<br>"
            "稀釋劑總用量：%{customdata[1]:,.1f} kg<br>"
            "歷史紀錄數：%{customdata[2]:,.0f}<br>"
            "試用優先分數：%{customdata[4]:.1f}"
            "<extra></extra>"
        )
    )

    fig_stability.add_vline(
        x=stable_coverage_limit * 100,
        line_dash="dash",
        line_width=2,
        line_color="black",
        annotation_text=(
            f"穩定率下限 {stable_coverage_limit:.0%}"
        ),
        annotation_position="top right"
    )

    fig_stability.update_xaxes(
        title="添加比例穩定率（%）",
        range=[0, 110],
        showline=True,
        linewidth=1.5,
    ...
    fig_stability.update_layout(
        plot_bgcolor="white",
        paper_bgcolor="white",
        margin=dict(l=100, r=170, t=105, b=75),
        legend_title_text="評估結果",
        legend=dict(
            orientation="h",
            yanchor="bottom",
    ...
    exported_figs[
        "10. Paint Code Solvent Ratio Stability Rate"
    ] = fig_stability
